## Step 2: eval_milan_boot.R
## bootstrap_indices.rds를 읽어 MILAN.0 모델의 bootstrap 평가 수행
...
data_cr <- data.frame(
  ...
  milan     = as.numeric(test_data$milan)
)
...
compute_metrics <- function(d) {
  ...
  auroc <- if (!is.null(sc))
    sc$AUC$score[model == "MILAN", AUC]   else rep(NA_real_, length(eval_times))
  bs    <- if (!is.null(sc))
    sc$Brier$score[model == "MILAN", Brier] else rep(NA_real_, length(eval_times))

  ## IBS via trapz over all event times
  event_times <- subset(d, fstatus %in% c(1, 2))$ftime

  ibs <- tryCatch({
    bs_all <- Score(list("MILAN" = d$milan),
                    formula = Hist(ftime, fstatus) ~ 1,
                    data    = d,
                    times   = event_times,
                    metrics = "Brier",
                    cause   = 1)
    dt  <- bs_all$Brier$score[model == "MILAN", .(times, Brier)]
...
saveRDS(list(boot_results = boot_results,
        ...
        "milan_boot_results.rds")

cat("\nSaved → milan_boot_results.rds\n")
