...
compute_wolbers_cindex <- function(model, test_data) {
    ...
    c_index <- pec::cindex(model,
			   ...
			   cause=1,
			   verbose=FALSE)
	return(c_index$AppCindex$rfsrc)
...
cpi_rsf_wolbers <- function(rsf,
    ...
    baseline_cindex <- compute_wolbers_cindex(rsf, data_val)
    ...
    for (j in seq_along(features)) {
        ...
        for (b in 1:B) {
            ...
            perm_cindex <- compute_wolbers_cindex(rsf, df_perm)
...
res <- cpi_rsf_wolbers(rsf = loaded_rsf,
		       data_val = test_data,
		       timeVar = "ftime",
		       statusVar = "fstatus",
...
print(str(res))
saveRDS(res, "cpi_rsf_wolbers_results.rds")
