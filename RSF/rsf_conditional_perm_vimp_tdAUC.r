...
# 결과 해석 함수들
interpret_importance <- function(cpi_results, min_effect_size = 0.001) {
...
plot_importance_heatmap <- function(interpretation_results,
	...
	# AUC heatmap 저장
	auc_filename <- paste0(filename_prefix, "_auc_rsf.pdf")
	...
	# Brier heatmap 저장
	brier_filename <- paste0(filename_prefix, "_bs_rsf.pdf")
...
# rsf      : 학습된 randomForestSRC 객체 (rfsrc 반환값)
...
cpi_rfsrc <- function(rsf,
	...
	pred <- predict(rsf, newdata=data_val)
	# calculate the cumulative incidence function at eval_times via interpolate
	pred_df <- interp_predict_risk_rsf(pred, eval_times)
	# calculating the baseline score before permutation
	base <- compute_metrics(pred_df, data_val, eval_times)
	M0_a <- base$auc
	M0_b <- base$brier
	...
	for (j in seq_along(features)) {
		...
		for (b in 1:B) {
			...
			df_perm[, c(varj, Sj)] <- conditional_permute_knn(df = data_val[, c(varj, Sj), drop = FALSE],
									  ...
									  seed = seed + 1000 * j  + b)
			# prediction after conditional permutation
			pred <- predict(rsf, newdata=df_perm)     
			# calculate the cumulative incidence function at eval_times via interpolate
			pred_df <- interp_predict_risk_rsf(pred, eval_times)
			# calculating the scores after conditional permutation
			mb <- compute_metrics(pred_df, df_perm, eval_times)
...
loaded_rsf <- readRDS("fitted_rsf.rds")
print(loaded_rsf$xvar.names)
...
res <- cpi_rfsrc(rsf = loaded_rsf,
		 data_val = test_data,
		 timeVar = "ftime",
		 statusVar = "fstatus",
		 features = loaded_rsf$xvar.names,
		 eval_times = t_stars,
		 B = 50,
		 k = 30,
		 alpha = 0.05,
...
var_name_mapping <- c(
  ...
  "log_lr_tr_v_size_whole_ct" = "Diameter of largest viable nodule by RECIST v1.1",
  "cr" = "Serum creatinine",
  "age_lt" = "Age",
  "hbv" = "HBV infection", 
  "ldlt" = "Living donor liver transplantation",
  "lr_5_ct" = "Number of LR-5 nodule",
  "lr_tr_v_ct" = "Number of LR-TR-V nodule",
  "lr_m_ct" = "Number of LR-M nodule",
  ...
  "lr_tr_e_ct" = "Number of LR-TR-E nodule",
  "lr_tr_nv_ct" = "Number of LR-TR-NV nodule",
  "log_afp" = "Serum AFP",
...
print(str(res))
saveRDS(res, "cpi_rsf_results.rds")


# 2. 결과 해석
interpretation <- interpret_importance(res, min_effect_size = 0.001)
