...
brier_scores <- Score(list("MILAN"=data_cr$milan),
		      formula=Hist(ftime, fstatus) ~ 1,
		      data=data_cr,
...
milan_brier_scores <- brier_scores_dt[model=="MILAN", .(times, Brier)]
ibs_milan <- trapz(milan_brier_scores$times, milan_brier_scores$Brier)
ibs <- ibs_milan/max(total_event_time_ibs)
...
saveRDS(roc_by_times, file="ext_milan_roc_by_times.rds")
