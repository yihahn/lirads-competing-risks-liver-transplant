## Step 2: eval_mt2.0_boot.r
## bootstrap_indices.rds를 읽어 mt2.0 모델의 bootstrap 평가 수행
...
data_cr <- data.frame(
  ...
  mt2     = as.numeric(test_data$mt2.0)
)
...
compute_metrics <- function(d) {
  ...
  auroc <- if (!is.null(sc))
    sc$AUC$score[model == "mt2", AUC]   else rep(NA_real_, length(eval_times))
  bs    <- if (!is.null(sc))
    sc$Brier$score[model == "mt2", Brier] else rep(NA_real_, length(eval_times))

  ## IBS via trapz over all event times
  event_times <- subset(d, fstatus %in% c(1, 2))$ftime

  ibs <- tryCatch({
    bs_all <- Score(list("mt2" = d$mt2),
                    formula = Hist(ftime, fstatus) ~ 1,
                    data    = d,
                    times   = event_times,
                    metrics = "Brier",
                    cause   = 1)
    dt  <- bs_all$Brier$score[model == "mt2", .(times, Brier)]
...
saveRDS(list(boot_results = boot_results,
        ...
        "mt2_boot_results.rds")

cat("\nSaved → mt2_boot_results.rds\n")
